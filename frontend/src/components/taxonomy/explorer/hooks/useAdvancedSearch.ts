...
import { mapSearchResultsPayload, sanitizeAdvancedFilters } from "../explorerDataUtils";
import { EMPTY_ADVANCED_FILTERS } from "../explorerTypes";
import { searchConcepts } from "../services/explorerApi";

type UseAdvancedSearchArgs = {
  year: string | null;
  entrypoint: string | null;
};

export function useAdvancedSearch({ year, entrypoint }: UseAdvancedSearchArgs) {
    ...
    async (nextOffset?: number) => {
      ...
      try {
        const payload = await searchConcepts({
          ...
          href: entrypoint,
          q: trimmedQuery,
          filters: latestAdvancedFiltersRef.current as unknown as Record<string, unknown>,
